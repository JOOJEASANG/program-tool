...
- 렌더러 래퍼 순서와 글자·책등 포인터 캡처 순서를 회귀 검사로 고정했다. 현재 선택 대상별 이벤트 중단은 의도된 범위에서 작동하며 최종 렌더 상태는 안전 경계가 복구한다.
- `cover-editor-multiselect.js`와 `cover-editor-layer-style.js`는 현재 기능을 제공하지 않는 비활성 호환 정리 파일임을 검사로 고정했다. 소스 HTML에서 제거하는 작업은 본체 HTML 구조 정리 단계로 남긴다.

## 다음 단계에서 정리할 항목

- 비활성 호환 파일 두 개의 소스 HTML 운영 로드를 제거한다.
- 표지 렌더러를 여러 파일이 연속 감싸는 구조를 장기적으로 명시적 단일 합성 진입점으로 축소한다.
