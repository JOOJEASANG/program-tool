...
6. `cover-editor-preflight-project.js`
7. `cover-editor-image-tools.js`

기능이 없던 `cover-editor-multiselect.js`와 `cover-editor-layer-style.js`는 HTML 소스에서 제거해 네트워크 요청 자체를 없앴다. jsPDF도 본체 HTML에서 제거하고 PDF 출력 버튼을 실제로 누른 시점에만 불러온다.
...
`cover-output-performance-safety.js`는 완성 가로·세로와 실제 출력 DPI로 캔버스 픽셀 수와 예상 작업 메모리를 계산한다. 기존 절대 한도 5,200만 픽셀 안에서 기기 메모리별 안전 한도를 적용하며, 캔버스 한 변 16,384px 초과도 사전에 차단한다. PDF 또는 가이드 PDF 버튼을 누른 경우에만 jsPDF를 불러오며 12초 안에 성공하거나 명확한 실패 상태로 종료한다. PNG 확인은 jsPDF 네트워크 요청 없이 동작한다.
...
`cover-render-pipeline-contract.js`는 모든 기존 렌더 보조 모듈의 설치가 끝난 뒤 현재 합성 렌더러를 한 번 캡처하고 `window.renderCover`의 최종 소유자를 하나로 고정한다. 이후 전역 렌더 함수가 다시 바뀌면 자동 재포장하지 않고 소유권 이탈만 경고해 재귀 래퍼 위험을 피한다. 비활성 스크립트 노드 정리는 오래된 HTML이 남은 환경을 위한 호환 안전장치로만 유지한다.
...
- jsPDF 연결 실패 시 출력 시점에 복구하고, 중복 로드와 무한 대기를 막는다.
...
- 기능이 없는 호환 스크립트 두 개를 본체 HTML에서 제거해 로딩과 실행 비용을 없앴다.
- 본체 HTML의 jsPDF 동기 CDN 태그를 제거했다.
- PDF·가이드 PDF 버튼을 누를 때만 `cover-output-performance-safety.js`가 jsPDF를 지연 로드한다.
- PNG 미리보기는 PDF 라이브러리를 불러오지 않는다.
- 본체 HTML의 예시 제목·연도·책등 문구를 빈 입력과 안내 문구로 바꿔 초기 화면 깜빡임과 자동저장 오염 가능성을 줄였다.
- 회귀 검사로 삭제된 스크립트와 샘플 기본값이 다시 들어오지 못하게 고정했다.
