...
def test_security_bridge_routes_encrypt_decrypt_over_20mb_to_storage_with_500mb_ceiling():
    ...
    assert "MAX_FILE_BYTES=500*1024*1024" in policy
    assert "MAX_TOTAL_BYTES=800*1024*1024" in policy
    assert "500MB" in BRIDGE


def test_security_bridge_uses_storage_endpoint_cleanup_and_extended_timeout():
    source = compact(BRIDGE)
...
def test_single_upload_policy_rejects_above_500mb_and_job_above_800mb():
    policy = compact(POLICY)
    assert "MAX_FILE_BYTES=500*1024*1024" in policy
    assert "MAX_TOTAL_BYTES=800*1024*1024" in policy
    assert "Number(file.size||0)>MAX_FILE_BYTES" in policy
    assert "nextTotal>MAX_TOTAL_BYTES" in policy
    assert "PDF한파일은최대500MB" in policy
    assert "전체합계는최대800MB" in policy


def test_backend_and_transient_storage_rules_are_aligned_to_500mb():
