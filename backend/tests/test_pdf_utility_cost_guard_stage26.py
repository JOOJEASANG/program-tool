...
MAIN=ROOT/"backend"/"main.py"; STORAGE_RULES=ROOT/"storage.rules"; DELIVERY=ROOT/"backend"/"utils"/"storage_delivery.py"; SECURITY=ROOT/"backend"/"routers"/"pdf_large_security.py"; FINALIZER=ROOT/"js"/"pdf-utility-finalize.js"; UTILITY_POLICY=ROOT/"js"/"pdf-utility-cost-guard-v2.js"; SECURITY_BRIDGE=ROOT/"js"/"pdf-utility"/"security-large-file.js"; PREFLIGHT_RUNTIME=ROOT/"js"/"pdf-preflight"/"route-runtime.js"; SW_REGISTER=ROOT/"js"/"sw-register.js"; INDEX=ROOT/"index.html"; GLOBAL_UI=ROOT/"js"/"program-studio-ui-v2.js"; LARGE_DIRECT=ROOT/"js"/"pdf-suite"/"direct-tool-large-storage.js"
def compact(v:str)->str:return "".join(v.split())
def test_pdf_utility_storage_policy_is_500mb_per_file_and_800mb_per_job_while_editor_stays_bounded():
 ...
 for marker in ("PDF_STORAGE_FILE_BYTES = 200 * MIB","PDF_STORAGE_TOTAL_BYTES = 300 * MIB","PDF_UTILITY_FILE_BYTES = 500 * MIB","PDF_UTILITY_TOTAL_BYTES = 800 * MIB","pdf_router.MAX_PDF_FILE_BYTES = PDF_STORAGE_FILE_BYTES","pdf_router.MAX_TOTAL_PDF_BYTES = PDF_STORAGE_TOTAL_BYTES","preflight_router.MAX_STORAGE_PDF_BYTES = PDF_UTILITY_FILE_BYTES","pdf_utility_router.MAX_FILE_BYTES = PDF_UTILITY_FILE_BYTES","pdf_utility_router.MAX_TOTAL_BYTES = PDF_UTILITY_TOTAL_BYTES"): assert marker in main
 assert "validPdfUpload(524288000)" in rules and "validPdfUpload(209715200)" in rules
 for marker in ("constMAX_FILES=10","constMAX_FILE_BYTES=500*1024*1024","constMAX_TOTAL_BYTES=800*1024*1024","nextTotal>MAX_TOTAL_BYTES","전체합계는최대800MB"): assert marker in policy
def test_high_cost_pdf_work_gets_more_memory_but_stays_instance_and_raster_bounded():
 main=MAIN.read_text(encoding="utf-8")
 for marker in ("memory=options.MemoryOption.GB_4","timeout_sec=600","min_instances=0","max_instances=2","pdf_utility_router.MAX_BACKGROUND_PAGES = 100","pdf_utility_router.MAX_BACKGROUND_PIXELS = 90_000_000","pdf_utility_router.BACKGROUND_DPI = 160",'schedule="every 1 hours"',"timedelta(hours=1)",'schedule="every 24 hours"',"MAX_SAVED_PDF_SESSIONS = 10","MAX_SAVED_DESIGN_PROJECTS = 8","ORPHAN_GRACE_HOURS = 24"): assert marker in main
def test_generated_pdf_delivery_is_800mb_bounded_and_one_hour_retention():
 source=DELIVERY.read_text(encoding="utf-8")
 for marker in ("RESULT_TTL_HOURS = 1","MAX_RESULT_BYTES = 800 * 1024 * 1024","size_bytes > MAX_RESULT_BYTES","최대 800MB",'"expiration_mode": "scheduled-delete"','"size_bytes": size_bytes'): assert marker in source
def test_large_encrypt_decrypt_uses_500mb_storage_bridge_and_direct_http_remains_small():
 ...
 for marker in ("MAX_FILE_BYTES=500*1024*1024","MAX_PAGES=2000",'operationnotin{"encrypt","decrypt"}'): assert marker in backend
 ...
 assert "DIRECT_MAX=20*1024*1024" in bridge and "MAX_FILE_BYTES=500*1024*1024" in bridge and "500MB" in SECURITY_BRIDGE.read_text(encoding="utf-8")
