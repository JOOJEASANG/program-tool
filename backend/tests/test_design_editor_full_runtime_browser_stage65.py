...
def test_stage65_browser_smoke_collects_every_production_runtime_result():
    ...
    assert "runtimeBoot.manifest.length===32" in source
    ...
    assert "DesignEditorCoverModel" in source
    assert "DesignEditorEmbeddedPolish" in source
...
def test_stage65_manifest_contract_and_browser_contract_keep_same_expected_count():
    ...
    assert manifest_block.count("['designEditor") == 32
    assert "runtimeBoot.manifest.length===32" in harness
