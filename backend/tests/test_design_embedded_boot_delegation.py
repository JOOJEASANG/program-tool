...
def test_deployment_version_busts_cached_boot_guard_after_embedded_fix():
    ...
    assert '"version": "2026.09.02.002"' in version
    assert '"label": "PDF 검사 부팅 스피너 수정, Firebase 스크립트 선행 로딩"' in version
    assert "const VERSION='2026.09.02.002'" in sw_register
    assert "const APP_VERSION='2026.09.02.002'" in sw
    assert "/js/sw-register.js?v=2026.09.02.002" in firebase
