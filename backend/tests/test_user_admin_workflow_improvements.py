...
def test_runtime_boot_uses_canonical_owners_for_live_workflow_modules():
    runtime=read("js/sw-register.js"); app_version=read("js/app-version.js"); pdf_editor=read("js/pdf-editor/route-runtime.js"); preflight=read("js/pdf-preflight/route-runtime.js")
    ...
    for asset in ("/js/pdf-editor-final-check.js","/js/pdf-print-readiness.js","/js/pdf-editor/spread-split.js","/js/pdf-editor/booklet-sheet-preview.js"): assert asset not in executable_app
...
def test_release_version_is_synchronized_for_new_workflow():
    version=json.loads(read("version.json")); expected=str(version["version"]).strip(); sw=read("sw.js"); firebase=read("js/firebase-config.js")
    assert expected
