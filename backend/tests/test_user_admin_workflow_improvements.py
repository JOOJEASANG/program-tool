...
def test_runtime_boot_uses_canonical_owners_for_live_workflow_modules():
 runtime=read("js/sw-register.js");app_version=read("js/app-version.js");pdf_editor=read("js/pdf-editor/route-runtime.js");preflight=read("js/pdf-preflight/route-runtime.js");assert "/js/admin-operations-overview.js" in runtime and "home-print-workflow.js" not in runtime
...
def test_admin_operations_overview_remains_explicit_and_non_destructive():
 source=read("js/admin-operations-overview.js"); assert "professional_program_suite" in source and "완성 도구 상태 정리" in source and "if(!confirm(" in source and "window.AdminProfessionalProgramManager?.reload?.()" in source and "$('aopSync').addEventListener('click',syncCanonical)" in source
