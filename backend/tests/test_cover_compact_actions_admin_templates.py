...
def test_cover_template_surface_is_fully_removed_even_for_stale_cache():
    ...
    for marker in (
        ...
        "template-surface-fully-removed",
        "표지 템플릿",
        "제공 이미지 템플릿",
    ):
        assert marker in source
    assert "card.replaceChildren" not in source
    assert "admin-image-template-only" not in source


def test_cover_runtime_loads_template_removal_before_compact_dock():
