...
def _draw_extra_text(page: fitz.Page, item: dict):
    ...
    page.insert_text(fitz.Point(start_x, baseline), text, fontsize=size, fontname=CJK_FONT_NAME, color=color, fill=color, fill_opacity=opacity, stroke_opacity=opacity, render_mode=render_mode, border_width=0.25 if render_mode else 1, morph=_text_morph(anchor, italic, rotation), overlay=True)
...
def render_divider_page(out_doc: fitz.Document, content_raw: str, style: str, paper_w_pt: float, paper_h_pt: float):
    ...
    if not no_bg and resolved_style == "band":
        page.draw_rect(fitz.Rect(0, paper_h_pt * 0.34, paper_w_pt, paper_h_pt * 0.66), color=None, fill=fg, fill_opacity=0.16, overlay=True)
