"""Professional crop marks and reserved bleed workspace for generated PDFs."""
...
def add_print_marks(pdf_bytes: bytes, settings) -> bytes:
    """Wrap each finished page in a larger media box with trim/bleed boxes."""
    ...
    try:
        ...
        for page_index in range(source.page_count):
            ...
            output_page = output.new_page(
                width=trim_width + outer * 2,
                height=trim_height + outer * 2,
            )
...
def apply_print_marks_if_enabled(pdf_bytes: bytes, request) -> bytes:
    settings = getattr(request, "print_marks", None)
    if not settings or not bool(getattr(settings, "enabled", False)):
        return pdf_bytes
    return add_print_marks(pdf_bytes, settings)


def rewrite_path_with_print_marks(path: str | Path, request) -> Path:
    destination = Path(path)
    settings = getattr(request, "print_marks", None)
    if not settings or not bool(getattr(settings, "enabled", False)):
        return destination
    marked = add_print_marks(destination.read_bytes(), settings)
    destination.write_bytes(marked)
    return destination
