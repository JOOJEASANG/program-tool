...
_ALLOWED_POSITIONS = {'top-left', 'top-right', 'bottom-left', 'bottom-right', 'bottom-center'}
...
def parse_numbering_options(raw) -> NumberingOptions:
    ...
    try:
        ...
        raise ValueError('넘버링 시작번호와 글자 크기를 확인해 주세요') from exc
    ...
    return NumberingOptions(True, start, fmt, position, font_size)
...
def _draw_number(page: fitz.Page, placement: Placement, label: str, options: NumberingOptions) -> None:
    ...
    base_width = max(1.0, fitz.get_text_length(label, fontname='helv', fontsize=font_size))
    available_width = max(6.0, rect.width - 2 * inset - 2 * pad_x)
    available_height = max(6.0, rect.height - 2 * inset - 2 * pad_y)
    ...
    if options.position.endswith('right'):
        x0 = rect.x1 - inset - box_width
    elif options.position == 'bottom-center':
