...
def _request_id() -> str:
    ...
    cached = getattr(g, "pdf_security_request_id", None)
    if isinstance(cached, str) and cached:
        return cached
    supplied = (request.headers.get("X-Request-ID") or "").strip()
    request_id = supplied if re.fullmatch(r"[A-Za-z0-9._-]{8,64}", supplied) else uuid.uuid4().hex[:16]
    g.pdf_security_request_id = request_id
    return request_id
