(function () {
  ...
  function lines(business, includePrivacyOfficer) {
    const values = [
      ...
      (business.bizNumber || business.bizRegNo) && `사업자등록번호: ${business.bizNumber || business.bizRegNo}`,
      business.bizMailOrder && `통신판매업 신고번호: ${business.bizMailOrder}`,
      (business.bizAddress || business.bizAddr) && `주소: ${business.bizAddress || business.bizAddr}`,
  ...
  async function render(options = {}) {
    ...
    try {
      ...
      if (effective) effective.textContent = business.bizTermsEffective || options.defaultEffectiveDate || '-';
      return business;
    } catch (error) {
      console.error(error);
