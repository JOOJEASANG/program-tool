...
(function(){
  ...
  function setProgress(percent,icon,message){const box=$('progressBox');if(box)box.style.display='block';if(typeof window.setProgress==='function')window.setProgress(Math.max(0,Math.min(100,Math.round(percent))),icon,message);}
  ...
  function installStyles(){
    ...
    style.id='pdfUtilityVisualOrganizerStyles';
    style.textContent=`
  ...
  function ensurePdfJs(){
    ...
    pdfJsPromise=new Promise((resolve,reject)=>{
      const existing=document.querySelector('script[data-pdf-visual-lib="pdfjs"]');
      const finish=()=>{if(window.pdfjsLib?.getDocument){window.pdfjsLib.GlobalWorkerOptions.workerSrc=PDFJS_WORKER;resolve(window.pdfjsLib);}else reject(new Error('PDF 썸네일 엔진 초기화에 실패했습니다.'));};
      if(existing){const started=Date.now();const wait=()=>{if(window.pdfjsLib?.getDocument)return finish();if(Date.now()-started>15000)return reject(new Error('PDF 썸네일 엔진 로딩 시간이 초과되었습니다.'));setTimeout(wait,50);};wait();return;}
      const script=document.createElement('script');script.src=PDFJS_SRC;script.async=true;script.crossOrigin='anonymous';script.referrerPolicy='no-referrer';script.dataset.pdfVisualLib='pdfjs';
      const timer=setTimeout(()=>reject(new Error('PDF 썸네일 엔진 로딩 시간이 초과되었습니다.')),15000);
      script.onload=()=>{clearTimeout(timer);finish();};script.onerror=()=>{clearTimeout(timer);reject(new Error('PDF 썸네일 엔진을 불러오지 못했습니다.'));};document.head.appendChild(script);
  ...
  function ensurePdfLib(){
    ...
    pdfLibPromise=new Promise((resolve,reject)=>{
      const existing=document.querySelector('script[data-pdf-visual-lib="pdf-lib"]');
      const finish=()=>window.PDFLib?.PDFDocument?resolve(window.PDFLib):reject(new Error('로컬 PDF 편집 엔진 초기화에 실패했습니다.'));
      if(existing){const started=Date.now();const wait=()=>{if(window.PDFLib?.PDFDocument)return finish();if(Date.now()-started>15000)return reject(new Error('로컬 PDF 편집 엔진 로딩 시간이 초과되었습니다.'));setTimeout(wait,50);};wait();return;}
      const script=document.createElement('script');script.src=PDFLIB_SRC;script.async=true;script.crossOrigin='anonymous';script.referrerPolicy='no-referrer';script.dataset.pdfVisualLib='pdf-lib';
      const timer=setTimeout(()=>reject(new Error('로컬 PDF 편집 엔진 로딩 시간이 초과되었습니다.')),15000);script.onload=()=>{clearTimeout(timer);finish();};script.onerror=()=>{clearTimeout(timer);reject(new Error('로컬 PDF 편집 엔진을 불러오지 못했습니다.'));};document.head.appendChild(script);
  ...
  function syncCardOrder(){
    ...
    for(const entry of state.pages){const card=grid.querySelector(`[data-source-page="${entry.sourcePage}"]`);if(card)grid.appendChild(card);}
    state.pages.forEach((entry,index)=>{
      const card=grid.querySelector(`[data-source-page="${entry.sourcePage}"]`);if(!card)return;
      const badge=card.querySelector('.pdfu-page-badge');if(badge)badge.textContent=String(index+1);
      const left=card.querySelector('[data-action="left"]');const right=card.querySelector('[data-action="right"]');if(left)left.disabled=index===0;if(right)right.disabled=index===state.pages.length-1;
    });
    syncSummary();
  }

  function movePage(sourcePage,toIndex){
    const from=findIndex(sourcePage);if(from<0)throw planError('이동할 페이지를 찾을 수 없습니다.');
    const target=Math.max(0,Math.min(state.pages.length-1,Number(toIndex)));
    if(from===target)return snapshot();
    const [entry]=state.pages.splice(from,1);state.pages.splice(target,0,entry);syncCardOrder();return snapshot();
  }

  function rotatePage(sourcePage,delta){
    const index=findIndex(sourcePage);if(index<0)throw planError('회전할 페이지를 찾을 수 없습니다.');
    state.pages[index].rotation=normalizeRotation(state.pages[index].rotation+Number(delta||0));
    const card=$('pdfUtilityVisualGrid')?.querySelector(`[data-source-page="${sourcePage}"]`);if(card)applyCardRotation(card,state.pages[index]);syncSummary();return snapshot();
  }

  function deletePage(sourcePage){
    if(state.pages.length<=1)throw planError('PDF에는 최소 1페이지가 남아 있어야 합니다.');
    const index=findIndex(sourcePage);if(index<0)throw planError('삭제할 페이지를 찾을 수 없습니다.');
    state.pages.splice(index,1);$('pdfUtilityVisualGrid')?.querySelector(`[data-source-page="${sourcePage}"]`)?.remove();syncCardOrder();return snapshot();
  }
  ...
  function syncSummary(){
    const summary=$('pdfUtilityVisualSummary');if(summary)summary.textContent=`현재 ${state.pages.length}페이지 / 원본 ${state.pageCount}페이지${planChanged()?' · 변경됨':' · 원본 순서'}`;
  }
  function applyCardRotation(card,entry){
    const canvas=card.querySelector('canvas');if(canvas)canvas.style.transform=`rotate(${entry.rotation}deg) scale(${entry.rotation%180===0?1:.78})`;
    const tag=card.querySelector('.pdfu-page-rotation');if(tag){tag.textContent=entry.rotation?`${entry.rotation}°`:'0°';tag.hidden=!entry.rotation;}
  }

  async function renderThumbnail(sourcePage,canvas,serial){
    const doc=state.pdfDoc;if(!doc||serial!==sessionSerial)return;
    try{
      const page=await doc.getPage(sourcePage);if(serial!==sessionSerial)return;
      const base=page.getViewport({scale:1});const scale=Math.min(128/Math.max(1,base.width),150/Math.max(1,base.height),1.25);const viewport=page.getViewport({scale});
      canvas.width=Math.max(1,Math.round(viewport.width));canvas.height=Math.max(1,Math.round(viewport.height));const context=canvas.getContext('2d');
  ...
  function createCard(entry,index,serial){
    const card=document.createElement('div');card.className='pdfu-page-card';card.draggable=true;card.dataset.sourcePage=String(entry.sourcePage);card.tabIndex=0;card.setAttribute('aria-label',`원본 ${entry.sourcePage}페이지, 현재 순서 ${index+1}`);
    card.innerHTML=`<div class="pdfu-page-preview"><span class="pdfu-page-badge">${index+1}</span><span class="pdfu-page-rotation" hidden>0°</span><canvas aria-hidden="true"></canvas></div><div class="pdfu-page-source">원본 ${entry.sourcePage}p</div><div class="pdfu-page-actions"><button type="button" class="pdfu-page-action" data-action="left" title="앞으로 이동" aria-label="앞으로 이동">←</button><button type="button" class="pdfu-page-action" data-action="right" title="뒤로 이동" aria-label="뒤로 이동">→</button><button type="button" class="pdfu-page-action" data-action="rotate-left" title="왼쪽 90도 회전" aria-label="왼쪽 90도 회전">↺</button><button type="button" class="pdfu-page-action" data-action="rotate-right" title="오른쪽 90도 회전" aria-label="오른쪽 90도 회전">↻</button><button type="button" class="pdfu-page-action delete" data-action="delete" title="페이지 삭제" aria-label="페이지 삭제">×</button></div>`;
    const canvas=card.querySelector('canvas');
    if('IntersectionObserver'in window){const observer=new IntersectionObserver(entries=>{if(entries.some(item=>item.isIntersecting)){observer.disconnect();renderThumbnail(entry.sourcePage,canvas,serial);}}, {root:$('pdfUtilityVisualGrid'),rootMargin:'180px'});observer.observe(card);}else renderThumbnail(entry.sourcePage,canvas,serial);
    return card;
  }

  function bindGrid(grid){
    grid.addEventListener('click',event=>{
      const button=event.target.closest('[data-action]');if(!button)return;const card=button.closest('.pdfu-page-card');const sourcePage=Number(card?.dataset.sourcePage||0);const index=findIndex(sourcePage);
      try{
        if(button.dataset.action==='left'&&index>0)movePage(sourcePage,index-1);
        if(button.dataset.action==='right'&&index>=0)movePage(sourcePage,index+1);
        if(button.dataset.action==='rotate-left')rotatePage(sourcePage,-90);
        if(button.dataset.action==='rotate-right')rotatePage(sourcePage,90);
        if(button.dataset.action==='delete')deletePage(sourcePage);
  ...
  function renderGrid(serial){
    const grid=$('pdfUtilityVisualGrid');if(!grid)return;grid.innerHTML='';state.pages.forEach((entry,index)=>grid.appendChild(createCard(entry,index,serial)));bindGrid(grid);syncSummary();
  }

  function buildModalShell(file){
    const overlay=$('pdfUtilityModalOverlay');const body=$('pdfUtilityModalBody');if(!overlay||!body)throw new Error('페이지 정리 창을 준비하지 못했습니다. 새로고침 후 다시 시도하세요.');
    installStyles();overlay.classList.add('pdfu-visual-open');$('pdfUtilityModalTitle').textContent='썸네일 페이지 정리';$('pdfUtilityModalDesc').textContent=`선택 파일: ${file.name} · 썸네일을 끌어 순서를 바꾸고 페이지별 회전·삭제를 할 수 있습니다.`;
    body.innerHTML='<div class="pdfu-visual-toolbar"><div><div class="pdfu-visual-summary" id="pdfUtilityVisualSummary">페이지 불러오는 중...</div><div class="pdfu-visual-help">드래그 또는 ← → 버튼으로 순서를 바꾸고, ↺ ↻ 버튼으로 90° 회전할 수 있습니다.</div></div><button type="button" class="pdfu-visual-reset" id="pdfUtilityVisualReset">원래대로</button></div><div class="pdfu-visual-grid" id="pdfUtilityVisualGrid"><div class="pdfu-visual-loading"><strong>페이지 썸네일 준비 중</strong>PDF는 브라우저에서 읽으며 미리보기를 위해 서버에 업로드하지 않습니다.</div></div><div class="pdfu-warning"><strong>저장 방식:</strong> '+mb(localLimit())+' 이하에서는 서버 업로드 없이 브라우저에서 저장합니다. 큰 파일이나 로컬 호환 오류가 있을 때만 인증된 서버 처리로 전환합니다.</div>';
    const run=$('pdfUtilityModalRun');run.textContent='정리한 PDF 저장';run.onclick=()=>saveCurrent();
    const close=()=>closeModal();$('pdfUtilityModalClose').onclick=close;$('pdfUtilityModalCancel').onclick=close;
    $('pdfUtilityVisualReset').onclick=()=>{resetPlan();renderGrid(sessionSerial);};overlay.classList.add('open');
  }

  async function openVisualOrganizer(){
    if(busy)return false;const file=activeFile();if(!file){showError('페이지를 정리할 PDF를 선택하세요.');return false;}
    sessionSerial+=1;const serial=sessionSerial;state.file=file;state.pdfDoc=null;state.pageCount=0;state.pages=[];
    try{
      buildModalShell(file);setMode('local',`${file.name} · 썸네일 미리보기는 브라우저에서 처리합니다.`);setProgress(5,'🗂️','페이지 썸네일 정보 읽는 중');
      const pdfjs=await ensurePdfJs();const bytes=await file.arrayBuffer();if(serial!==sessionSerial)return false;
      const loading=pdfjs.getDocument({data:bytes});state.pdfDoc=await loading.promise;const count=Number(state.pdfDoc?.numPages||0);if(!count)throw new Error('페이지가 없는 PDF입니다.');if(count>MAX_VISUAL_PAGES)throw planError(`썸네일 페이지 정리는 최대 ${MAX_VISUAL_PAGES}페이지까지 지원합니다. 페이지 범위 재정렬 기능을 이용하세요.`);
      resetPlan(count);renderGrid(serial);setProgress(100,'✅',`${count}페이지 썸네일 준비 완료`);showStatus(`${count}페이지를 브라우저에서 불러왔습니다. 서버 업로드 없음.`,'ok');stopProgressSoon();document.documentElement.dataset.pdfVisualOrganizer='local-first';return true;
  ...
  function validatePlan(){
    if(!state.file||!state.pageCount||!state.pages.length)throw planError('저장할 페이지 정리 정보가 없습니다.');const seen=new Set();for(const item of state.pages){if(!Number.isInteger(item.sourcePage)||item.sourcePage<1||item.sourcePage>state.pageCount)throw planError('페이지 순서 정보가 올바르지 않습니다.');if(seen.has(item.sourcePage))throw planError('같은 원본 페이지를 두 번 배치할 수 없습니다.');seen.add(item.sourcePage);item.rotation=normalizeRotation(item.rotation);}return snapshot();
  }

  async function saveLocally(file){
    const plan=validatePlan();const {PDFDocument,degrees}=await ensurePdfLib();setProgress(16,'🗂️','원본 PDF 읽는 중');const sourceBytes=await file.arrayBuffer();let source;
    try{source=await PDFDocument.load(sourceBytes,{ignoreEncryption:false,updateMetadata:false});}catch(error){if(/encrypted|password|decrypt|encryption/i.test(String(error?.message||'')))throw planError('암호화된 PDF는 먼저 암호를 해제하세요.');throw error;}
    if(Number(source.getPageCount?.()||0)!==state.pageCount)throw planError('PDF 페이지 수가 미리보기와 달라졌습니다. 파일을 다시 선택하세요.');
    setProgress(48,'🗂️','페이지 순서와 회전 적용 중');const output=await PDFDocument.create();const copied=await output.copyPages(source,plan.pages.map(item=>item.sourcePage-1));
    copied.forEach((page,index)=>{const item=plan.pages[index];if(item.rotation&&typeof page.setRotation==='function'&&typeof degrees==='function'){const base=Number(page.getRotation?.()?.angle||0);page.setRotation(degrees((base+item.rotation)%360));}output.addPage(page);});
    setProgress(84,'💾','정리 PDF 생성 중');const bytes=await output.save({useObjectStreams:true,addDefaultPage:false,updateFieldAppearances:false});return {blob:new Blob([bytes],{type:'application/pdf'}),pageCount:copied.length};
  }

  async function ensureStorage(){
    if(typeof window._ensureStorage==='function')return window._ensureStorage();if(window.firebase?.storage)return firebase.storage();
    await new Promise((resolve,reject)=>{const script=document.createElement('script');script.src='https://www.gstatic.com/firebasejs/9.23.0/firebase-storage-compat.js';script.onload=resolve;script.onerror=()=>reject(new Error('Firebase Storage SDK를 불러오지 못했습니다.'));document.head.appendChild(script);});return firebase.storage();
  }
  ...
  async function saveOnServer(file){
    ...
    try{
      setProgress(20,'📤','정리할 PDF 업로드 중');showStatus('대용량 또는 로컬 호환 문제로 인증된 서버 처리로 전환했습니다.','info');await storageInstance.ref(storagePath).put(file,{contentType:'application/pdf'});const token=await user.getIdToken(true);setProgress(54,'🗂️','서버에서 페이지 순서·회전 적용 중');
      const rotations={};for(const item of plan.pages)if(item.rotation)rotations[String(item.sourcePage)]=item.rotation;
      const response=await fetch('/api/pdf-utility/organize-storage',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({storage_path:storagePath,filename:file.name,page_order:plan.pages.map(item=>item.sourcePage),page_rotations:rotations}),signal:controller.signal});
      if(!response.ok){let message=`서버 오류 (${response.status})`;try{const payload=await response.json();message=payload?.detail||payload?.message||message;}catch(_){}throw new Error(message);}return {blob:await readPdfResponse(response,storageInstance),pageCount:Number(response.headers.get('X-PDF-Page-Count')||plan.pages.length)};
  ...
  function setBusy(value){busy=Boolean(value);const api=utility();if(api?.state)api.state.busy=busy;if(typeof window.setPageBusy==='function')window.setPageBusy(busy,'썸네일 페이지 정리');for(const id of ['pdfUtilityModalRun','pdfUtilityModalClose','pdfUtilityModalCancel','pdfUtilityVisualReset'])if($(id))$(id).disabled=busy;const button=$('pdfUtilityVisualOrganizeBtn');if(button&&!busy)button.disabled=!activeFile();}

  async function saveCurrent(){
    if(busy)return false;const file=state.file;try{validatePlan();}catch(error){showError(error.message);return false;}if(!planChanged()){showError('페이지 순서·회전·삭제 변경이 없습니다.');return false;}
    setBusy(true);
    try{
      const limit=localLimit();let result=null;
      if(Number(file.size||0)<=limit){setMode('local',`${file.name} · 정리한 PDF를 서버 업로드 없이 저장합니다.`);showStatus('페이지 순서·회전·삭제를 브라우저에서 직접 적용하는 중입니다. 서버 업로드 없음.','info');setProgress(5,'🗂️','브라우저 로컬 페이지 정리 준비 중');try{result=await saveLocally(file);document.documentElement.dataset.pdfVisualOrganizerLastResult='local-success';setMode('local-success',`${result.pageCount}페이지 · 서버 업로드 없이 페이지 정리를 완료했습니다.`);}catch(error){if(error?.code==='PDF_VISUAL_PLAN_INVALID')throw error;setMode('server-fallback',`${error?.message||'로컬 처리 호환 오류'} · 서버 처리로 계속 진행합니다.`);showStatus('브라우저 로컬 저장이 어려워 서버 처리로 자동 전환합니다.','info');}}
      else{setMode('server-fallback',`로컬 권장 한도 ${mb(limit)} 초과 (${mb(file.size)}) · 서버 처리합니다.`);showStatus(`파일이 로컬 권장 한도 ${mb(limit)}를 초과해 저장만 서버 처리로 전환합니다.`,'info');}
      if(!result){result=await saveOnServer(file);document.documentElement.dataset.pdfVisualOrganizerLastResult='server-success';}
      downloadBlob(result.blob,`${safeBaseName(file)}_페이지시각정리_${result.pageCount}p.pdf`);setProgress(100,'✅','페이지 정리 PDF 저장 완료');showStatus(`페이지 정리 완료 · ${result.pageCount}페이지${document.documentElement.dataset.pdfVisualOrganizerLastResult==='local-success'?' · 서버 업로드 없음':' · 서버 처리'}`,'ok');stopProgressSoon();closeModal();return true;
  ...
  window.ProgramStudioPdfVisualPageOrganizer=Object.freeze({version:'2026.09.04.001',strategy:'thumbnail-local-first-with-server-fallback',maxVisualPages:MAX_VISUAL_PAGES,openVisualOrganizer,saveCurrent,movePage,rotatePage,deletePage,resetPlan,snapshot,localLimitBytes:localLimit});
})();
