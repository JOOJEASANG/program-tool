...
(function () {
  'use strict';
  if (window.__pdfUtilityCostGuardV2) return;
  window.__pdfUtilityCostGuardV2 = true;
  ...
  function syncLargeSecurityButtons() {
    ...
    const title = '최대 500MB PDF까지 Storage 기반 암호 작업을 지원합니다.';
    for (const id of ['encryptBtn', 'decryptBtn']) {
      const button = $(id);
      if (!button) continue;
      if (!state()?.busy && button.disabled) button.disabled = false;
      if (button.title !== title) button.title = title;
    }
  }

  function rewriteLimitText() {
    const summaryText = `${state()?.files?.length || 0} / ${MAX_FILES} · ${mb(totalBytes())} / 500MB`;
    setTextIfChanged($('pdfUtilityFileSummary'), summaryText);
    setTextIfChanged(
      document.querySelector('.pdfu-limit-note'),
      'PDF는 최대 10개, 한 파일 최대 500MB, 한 번 작업 전체 합계 500MB까지 등록할 수 있습니다. 대용량 변환 작업은 서버 비용과 안정성을 위해 페이지·해상도 한도가 별도로 적용됩니다.'
    );
    setHtmlIfChanged(
      document.querySelector('.upload-sub'),
      '클릭하거나 여러 PDF를 끌어다 놓으세요.<br>최대 10개 · 파일당/전체 합계 500MB · PDF 형식만 지원'
    );
  ...
  function makeSecurityModal() {
    ...
    overlay.innerHTML = `
      <div class="pdfcost-box">
        ...
        <div class="pdfcost-note">20MB를 초과한 PDF는 Firebase Storage로 직접 전송해 처리합니다. 입력 임시파일과 다운로드 결과는 작업 직후 삭제를 시도하며, 비정상 종료 파일도 짧은 보존시간 뒤 자동 정리됩니다.</div>
  ...
  function openLargeSecurity(operation) {
    ...
    setTextIfChanged($('pdfCostSecurityTitle'), operation === 'encrypt' ? '대용량 PDF 암호 설정' : '대용량 PDF 암호 해제');
    setTextIfChanged($('pdfCostSecurityFile'), `${file.name} · ${mb(file.size)}`);
    ...
    setTextIfChanged($('pdfCostSecurityStatus'), '');
  ...
  function bindLargeSecurity() {
    ...
    if (run && run.dataset.bound !== '1') {
      ...
      run.addEventListener('click', async () => {
        ...
        setTextIfChanged($('pdfCostSecurityStatus'), '처리 중입니다. 대용량 파일은 시간이 걸릴 수 있습니다.');
        try {
          await runLargeSecurity(operation, password);
          overlay.classList.remove('open');
        } catch (error) {
          setTextIfChanged($('pdfCostSecurityStatus'), error?.message || '암호 작업에 실패했습니다.');
  ...
  function watchUi() {
    ...
    if (list) {
      new MutationObserver(() => queueMicrotask(rewriteLimitText)).observe(list, {
        childList: true,
        subtree: true,
        characterData: true
      });
    ...
    for (const id of ['encryptBtn', 'decryptBtn']) {
      const button = $(id);
      if (!button) continue;
      new MutationObserver(() => queueMicrotask(syncLargeSecurityButtons)).observe(button, {
        attributes: true,
  ...
  function install() {
    ...
    watchUi();
    rewriteLimitText();
    document.documentElement.dataset.pdfUtilityCostGuard = '2';
  }
