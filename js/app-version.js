...
(function(){
  ...
  function loadScopedScript(id,src,async=true){
    if(document.getElementById(id))return;
    const script=document.createElement('script');script.id=id;script.src=src;script.async=async;document.head.appendChild(script);
  }
  function loadCatalogScripts(target){
    loadScopedScript('programCatalogCoreScriptV1','/js/program-catalog-core.js?v=20260818-1',false);
    const script=document.getElementById('programCatalogCoreScriptV1');
    const loadTarget=()=>{
      if(target==='home')loadScopedScript('homeProgramCatalogScriptV1','/js/home-program-catalog.js?v=20260808-1',true);
      if(target==='admin'){
        loadScopedScript('adminProgramCatalogManagerScriptV1','/js/admin-program-catalog-manager.js?v=20260808-1',true);
        loadScopedScript('adminProgramIconPaletteScriptV1','/js/admin-program-icon-palette.js?v=20260808-1',true);
        loadScopedScript('adminProgramCatalogNavGuardScriptV1','/js/admin-program-catalog-nav-guard.js?v=20260818-1',true);
      }
    };
    if(window.ProgramCatalogCore)loadTarget();else if(script)script.addEventListener('load',loadTarget,{once:true});
  }
  function loadScopedEnhancements(){
    ...
    if(isPdfUtility){
      loadScopedScript('pdfUtilityPanelResizerScriptV2','/js/pdf-utility-panel-resizer.js?v=20260821-4',true);
      const loadOptional=()=>{
      ...
      if('requestIdleCallback' in window)window.requestIdleCallback(loadOptional,{timeout:2000});else setTimeout(loadOptional,400);
    }
  }
  function bootEnhancements(){
    if(isPdfUtility){if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',loadScopedEnhancements,{once:true});else setTimeout(loadScopedEnhancements,0);}else loadScopedEnhancements();
  }
  async function check(){
    try{
      const response=await fetch('/version.json?t='+Date.now(),{cache:'no-store',headers:{'Cache-Control':'no-cache'}});
      if(!response.ok)return;
      const data=await response.json();const currentVersion=String(data.version||'').trim();if(!currentVersion||currentVersion==='unknown')return;
      const previousVersion=localStorage.getItem(LOCAL_KEY)||'';localStorage.setItem(LOCAL_KEY,currentVersion);
