(function(){
  ...
  function isLoaded(item){
    ...
    const alias=String(item.fontFamily||'');
    if(!alias)return false;
    if(loadedAliases.has(alias))return true;
    try{return Boolean(document.fonts?.check?.(`16px ${alias}`));}catch(_){return false;}
  }

  function localFontItems(){
    const p=project();if(!p)return[];
    const items=[];
    for(const s of p.surfaces||[]){for(const item of s.elements||[]){if(item?.type==='text'&&item.localFontSource==='local')items.push(item);}}
    return items;
  }
  ...
  function renderLocalOptions(){
    ...
    const count=panel.querySelector('#designLocalFontCount');if(count)count.textContent=fontRecords.length?`${filtered.length.toLocaleString()} / ${fontRecords.length.toLocaleString()}개 표시`:'아직 PC 글꼴을 불러오지 않았습니다.';
  }
  ...
  function runtimeOption(select,item){
    ...
    if(!option){option=document.createElement('option');option.value=alias;option.textContent=`${itemLabel(item)} · 내 PC 글꼴`;select.appendChild(option);}
  ...
  function updatePanelState(item){
    ...
    if(local){selectedPostscript=String(item.localFontPostscriptName||'');switchTab(panel,'local');runtimeOption(byId('fontInput'),item);}else switchTab(panel,panel.dataset.tab==='local'?'local':'free');
  ...
  async function applyRecord(record,item=selectedText()){
    ...
    const select=byId('fontInput');
    runtimeOption(select,item);
    ...
    window.DesignEditorTypographyPro?.sync?.();
  ...
  async function reconnectProjectFonts(){
    const items=localFontItems();
    if(!items.length)return{loaded:0,missing:[]};
    ...
    for(const item of items){
      ...
      try{
        const alias=aliasFor(record);await loadFontFace(record,alias);item.fontFamily=alias;item.localFontRuntimeFamily=alias;loaded+=1;
  ...
  async function queryFonts(){
    ...
    if(typeof window.queryLocalFonts!=='function'){
      setStatus('이 브라우저는 PC 글꼴 목록 접근을 지원하지 않습니다. 데스크톱 Chrome/Edge에서 무료 폰트를 사용하거나 PC 글꼴 기능을 이용하세요.','err');
      return;
    }
    ...
    try{
      ...
      const item=selectedText();updatePanelState(item);
  ...
  function sync(){
    ...
    if(!item){byId(PANEL_ID)?.remove();return;}
    installPanel(item);
  }
