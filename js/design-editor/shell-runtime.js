...
await loadSupportScript('designSharedModuleProfileScriptV1','/js/design-editor/shared/module-profile.js?v=20260831-1');await loadBoundaryUi();await Promise.all(MODULES.map(entry=>loadEntry(entry)));await Promise.all([loadWorkspaceNavigation(),loadSidebarMenuOrder(),loadUiRevision(),loadDirectResize(),loadStabilityGuards(),loadFullPreviewWorkspace(),loadProductSpecificWorkspace()]);sync();await finalizeWorkspace();if(standalone){root.dataset.designStandaloneApp=activeProfile()?.key||rawApp||app;root.dataset.designStandaloneRuntime=activeProfile()?.runtimeProduct||app;}return true;
}
