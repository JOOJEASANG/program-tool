// Stabilize every PDF Utility right-stage transition and reject stale async overlays.
(function(){
  'use strict';
  if(window.__programStudioPdfUtilityWorkspaceStabilityV2)return;
  window.__programStudioPdfUtilityWorkspaceStabilityV2=true;
  ...
  function addClass(node,...names){if(!node)return;names.forEach(name=>{if(!node.classList.contains(name))node.classList.add(name);});}
  function removeClass(node,...names){if(!node)return;names.forEach(name=>{if(node.classList.contains(name))node.classList.remove(name);});}
  function stage(){return $('pdfUtilityStageBody');}
  function currentName(){return document.querySelector('[data-pdfu-tool].active .pdfu-menu-name')?.textContent?.trim()||window.ProgramStudioPdfSinglePageWorkspace?.currentTool||'';}
  function setSharedMode(enabled){const target=stage();if(!target)return;if(enabled)addClass(target,'pdfu-shared-advanced-stage');else removeClass(target,'pdfu-shared-advanced-stage');}
  ...
  function installStyle(){
    ...
    style.textContent=`
      ...
      @media(max-width:780px){.pdfu-local-workgrid,.pdfu-stage-body.pdfu-shared-advanced-stage{grid-template-columns:1fr}.pdfu-local-workgrid{min-height:0}.pdfu-local-result{min-height:360px}.pdfu-local-controls .local-actions{grid-template-columns:repeat(2,minmax(0,1fr))!important}.pdfu-shared-advanced-stage>#local-tools .pdfu-local-result{display:none}}
  ...
  function ensureLocalStructure(panel=$('local-tools')){
    if(!panel||panel.dataset.pdfuStableLayout==='1')return panel||null;
    const head=panel.querySelector('.local-head'),drop=panel.querySelector('#localDrop'),input=panel.querySelector('#localFile'),note=panel.querySelector('#localFileNote'),actions=panel.querySelector('.local-actions'),status=panel.querySelector('#localStatus'),metadata=panel.querySelector('#localMetadata');
  ...
  function expectedOverlayIds(name=currentName()){
    const ids=new Set();
    const shared=sharedSpec(name),inline=inlineSpec(name);
    if(shared?.overlay)ids.add(shared.overlay);
    if(inline?.overlay)ids.add(inline.overlay);
    if(OCR_TOOLS.has(name))ids.add('pdfSuiteOcrModal');
    return ids;
  }

  function closeOverlay(overlay){
    if(!overlay)return;
    removeClass(overlay,'open','pdfu-inline-overlay','pdfu-stable-inline');
    if(overlay.parentElement!==document.body)document.body.appendChild(overlay);
  }

  function closeInlineOverlays(){
    setSharedMode(false);
    document.querySelectorAll('.pdfadv-overlay.open,.pdfocr-overlay.open,.pdfadv-overlay.pdfu-inline-overlay,.pdfocr-overlay.pdfu-inline-overlay,.pdfadv-overlay.pdfu-stable-inline,.pdfocr-overlay.pdfu-stable-inline').forEach(closeOverlay);
  ...
  function wrapDirectBridge(){
    ...
    window.ProgramStudioPdfUtilityDirectBridge=Object.freeze({
      __workspaceStabilityWrapped:true,
      activate(tool){closeInlineOverlays();return bridge.activate?.(tool)===true;},
      handles:name=>bridge.handles?.(name),requiresAuth:name=>bridge.requiresAuth?.(name),
      reset(){closeInlineOverlays();return bridge.reset?.();},typeFor:name=>bridge.typeFor?.(name),
      stage:'pdf-utility-direct-tools-stable-v2'
    });
    document.documentElement.dataset.pdfUtilityDirectReset='stable';
  }

  function normalizeOverlay(overlay,name=currentName()){
    const target=stage();if(!target||!overlay?.classList.contains('open'))return false;
    if(!expectedOverlayIds(name).has(overlay.id)){closeOverlay(overlay);return false;}
    addClass(overlay,'pdfu-inline-overlay','pdfu-stable-inline');
    const shared=SHARED_ADVANCED.has(name);setSharedMode(shared);
    if(shared){
      ensureLocalStructure(target.querySelector('#local-tools')||$('local-tools'));
      if(overlay.parentElement!==target)target.appendChild(overlay);
    }else if(overlay.parentElement!==target||target.childElementCount!==1||target.firstElementChild!==overlay){
  ...
  function normalizeOpenOverlays(name=currentName()){
    const expected=expectedOverlayIds(name);
  ...
  function recoverAdvanced(name){
    ...
    const api=window.ProgramStudioPdfSuiteAdvanced;if(!api?.launch)return false;
  ...
  function recoverOcr(name){
    ...
    const api=window.ProgramStudioPdfSuiteOcr;if(!api?.open)return false;
    api.open(mode);setTimeout(()=>normalizeOpenOverlays(name),0);return true;
  }

  function stageHasContent(){
    const target=stage();if(!target)return false;
    return [...target.children].some(node=>{const style=getComputedStyle(node);return style.display!=='none'&&style.visibility!=='hidden'&&(node.getBoundingClientRect().height>1||node.textContent.trim());});
  }

  function recoveryCard(name){
    const target=stage();if(!target)return;setSharedMode(false);
    target.innerHTML=`<section class="pdfu-stage-recovery"><div><strong>${String(name||'PDF 기능')} 화면을 불러오지 못했습니다.</strong><p>기능 초기화가 지연되었습니다. 아래 버튼으로 해당 메뉴를 다시 연결할 수 있습니다.</p><button type="button" data-pdfu-retry>다시 연결</button></div></section>`;
    target.querySelector('[data-pdfu-retry]')?.addEventListener('click',()=>window.ProgramStudioPdfSinglePageWorkspace?.selectTool?.(name));
  ...
  function auditSelection(name,serial){
    ...
    const target=stage();if(!target)return;
    const local=target.querySelector('#local-tools');if(local)ensureLocalStructure(local);
    ...
    setTimeout(()=>{
      if(serial!==auditSerial||name!==currentName())return;
      const localNow=target.querySelector('#local-tools');if(localNow)ensureLocalStructure(localNow);
      normalizeOpenOverlays(name);
      if(!stageHasContent()){
        const retried=target.dataset.pdfuAuditRetried===name;
        if(!retried){target.dataset.pdfuAuditRetried=name;window.ProgramStudioPdfSinglePageWorkspace?.selectTool?.(name);setTimeout(()=>auditSelection(name,serial),80);}else recoveryCard(name);
      }else{
        delete target.dataset.pdfuAuditRetried;
        document.documentElement.dataset.pdfUtilityMenuAudit='ok';document.documentElement.dataset.pdfUtilityMenuAuditLast=name||'unknown';
      }
    },120);
  }

  function onMenuBeforeChange(event){
    const button=event.target.closest?.('[data-pdfu-tool]');if(!button)return;
    auditSerial+=1;
    setSharedMode(false);
    closeInlineOverlays();
    const target=stage();if(target)delete target.dataset.pdfuAuditRetried;
    document.documentElement.dataset.pdfUtilityMenuAudit='transition';
  }

  function onMenuAfterChange(event){
    const button=event.target.closest?.('[data-pdfu-tool]');if(!button)return;
  ...
  function observe(){
    const observer=new MutationObserver(()=>{
      const name=currentName(),local=stage()?.querySelector('#local-tools');if(local)ensureLocalStructure(local);normalizeOpenOverlays(name);
    });
  ...
  function install(){
    ...
    document.documentElement.dataset.pdfUtilityWorkspaceStabilityVersion='v2';
  }

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
  window.ProgramStudioPdfUtilityWorkspaceStability=Object.freeze({ensureLocalStructure,normalizeOpenOverlays,closeInlineOverlays,recoverAdvanced,recoverOcr,expectedOverlayIds,stage:'pdf-utility-workspace-stability-v2'});
})();