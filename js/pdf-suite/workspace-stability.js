...
(function(){
  ...
  function installStyle(){
    ...
    style.textContent=`
      ...
      @media(max-width:1040px){.pdfu-local-workgrid{grid-template-columns:minmax(270px,360px) minmax(0,1fr)}}
      @media(max-width:780px){.pdfu-local-workgrid{grid-template-columns:1fr;min-height:0}.pdfu-local-result{min-height:360px}.pdfu-local-controls .local-actions{grid-template-columns:repeat(2,minmax(0,1fr))!important}}
  ...
  function normalizeOpenOverlays(name=currentName()){
    ...
    if(document.body.style.overflow==='hidden'&&document.querySelector('.pdfu-stage-body .pdfadv-overlay.open,.pdfu-stage-body .pdfocr-overlay.open'))document.body.style.overflow='';
  }
  ...
  function stageHasContent(){
    ...
    return [...stage.children].some(node=>{
  ...
  function recoveryCard(name){
    ...
    stage.innerHTML=`<section class="pdfu-stage-recovery"><div><strong>${String(name||'PDF 기능')} 화면을 불러오지 못했습니다.</strong><p>기능 초기화가 지연되었습니다. 아래 버튼으로 해당 메뉴를 다시 연결할 수 있습니다.</p><button type="button" data-pdfu-retry>다시 연결</button></div></section>`;
    stage.querySelector('[data-pdfu-retry]')?.addEventListener('click',()=>window.ProgramStudioPdfSinglePageWorkspace?.selectTool?.(name));
  ...
  function auditSelection(name,serial){
    ...
    setTimeout(()=>{
      ...
      if(!stageHasContent()){
        ...
        delete stage.dataset.pdfuAuditRetried;
  ...
  function observe(){
    const observer=new MutationObserver(()=>{
      ...
      const local=$('pdfUtilityStageBody')?.querySelector('#local-tools');if(local)ensureLocalStructure(local);
      normalizeOpenOverlays(name);
    });
