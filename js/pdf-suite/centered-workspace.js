...
(function(){
  ...
  function mountServerTool(button,name){
    ...
    setStageHeader(isMerge?'PDF 합치기':'페이지 추출·나누기',isMerge?'합칠 PDF를 2개 이상 선택한 뒤 파일 목록을 드래그해 순서를 정합니다.':'페이지를 추출할 PDF 한 파일을 이 작업창에서 선택합니다.');
    ...
    const clearDropMarkers=()=>list.querySelectorAll('.drop-before,.drop-after,.dragging').forEach(node=>node.classList.remove('drop-before','drop-after','dragging'));
    const moveFile=(from,to)=>{
      if(!isMerge||from===to||from<0||to<0||from>=files.length||to>=files.length)return false;
      const [moved]=files.splice(from,1);files.splice(to,0,moved);render();return true;
    };
    const moveFileToBoundary=(from,boundary)=>{
      if(!isMerge||from<0||from>=files.length)return false;
      let target=Math.max(0,Math.min(files.length,boundary));
      if(from<target)target-=1;
      if(target===from)return false;
      const [moved]=files.splice(from,1);files.splice(target,0,moved);render();return true;
    };
    ...
    const render=()=>{
      ...
      files.forEach((file,index)=>{
        const row=document.createElement('div');row.className=`pdfuc-server-file${isMerge?' is-merge':''}`;row.dataset.pdfucFileIndex=String(index);row.draggable=isMerge;
        if(isMerge){
          row.innerHTML=`<span class="pdfuc-server-drag" aria-hidden="true" title="드래그하여 순서 변경">⋮⋮</span><strong>${index+1}. ${esc(file.name)}</strong><span>${humanBytes(file.size)}</span><span class="pdfuc-server-order"><button class="pdfuc-server-move" data-pdfuc-move="-1" type="button" aria-label="${esc(file.name)} 위로 이동" ${index===0?'disabled':''}>↑</button><button class="pdfuc-server-move" data-pdfuc-move="1" type="button" aria-label="${esc(file.name)} 아래로 이동" ${index===files.length-1?'disabled':''}>↓</button></span><button class="pdfuc-server-remove" type="button" aria-label="${esc(file.name)} 삭제">×</button>`;
          row.querySelectorAll('[data-pdfuc-move]').forEach(move=>move.addEventListener('click',()=>{const delta=Number(move.dataset.pdfucMove||0);moveFile(index,index+delta);}));
          row.addEventListener('dragstart',event=>{
            dragSourceIndex=index;row.classList.add('dragging');
          ...
          row.addEventListener('dragleave',event=>{if(event.relatedTarget instanceof Node&&row.contains(event.relatedTarget))return;row.classList.remove('drop-before','drop-after');});
          row.addEventListener('drop',event=>{
            if(dragSourceIndex<0)return;
            event.preventDefault();event.stopPropagation();
            const rect=row.getBoundingClientRect();const after=event.clientY>rect.top+rect.height/2;
            const from=dragSourceIndex;dragSourceIndex=-1;clearDropMarkers();moveFileToBoundary(from,index+(after?1:0));
          });
          row.addEventListener('dragend',()=>{dragSourceIndex=-1;clearDropMarkers();});
        }else{
          row.innerHTML=`<strong>${esc(file.name)}</strong><span>${humanBytes(file.size)}</span><button class="pdfuc-server-remove" type="button" aria-label="${esc(file.name)} 삭제">×</button>`;
        }
        row.querySelector('.pdfuc-server-remove').addEventListener('click',()=>{files.splice(index,1);dragSourceIndex=-1;render();});list.appendChild(row);
      });
      const total=files.reduce((sum,file)=>sum+Number(file.size||0),0);
