(function(){
  ...
  function setQuickActionsEnabled(enabled){
    byId('appQuickActions')?.querySelectorAll('button').forEach(button=>{
  ...
  function maybeReady(){
    if(accessGranted&&frameReady)ready();
  }
  ...
  function load(){
    ...
    setText('loadingMessage',app.workspaceHint||'공통 PDF 편집 엔진을 연결하는 중입니다.');
    const legacy=byId('legacyLink');
    if(legacy){
      legacy.href=app.legacy;
      legacy.hidden=false;
    }
    engineChip?.classList.add('loading');
    ...
    clearTimeout(timer);
    timer=setTimeout(
      ()=>fail('작업 엔진 응답이 늦습니다. 새로고침 후 다시 시도해 주세요.'),
      18000
    );
  }
  ...
  function startAfterAccess(){
    if(accessGranted)return;
    if(document.documentElement.dataset.accessReady==='true'){
      grantAccess();
      return;
    }
    ...
    if(access&&typeof access.then==='function'){
      ...
      Promise.resolve(observed).then(result=>{
        if(accessGranted)return;
        if(result||document.documentElement.dataset.accessReady==='true'){
          grantAccess();
          return;
        }
        retryAccess();
  ...
  frame?.addEventListener('load',()=>{
    ...
    try{
      ...
      if(!doc){
        markFrameReady('load-fallback');
        return;
      }
      const html=doc.documentElement;
      const win=frame.contentWindow;
      if(html?.dataset?.appReady==='true'||win?.PdfEditorCoreRuntime){
        markFrameReady('load');
        return;
      }
    }catch(_){}
  ...
  // Preload the protected PDF engine while the shared access guard resolves.
  // Visibility remains blocked until both access and frame readiness succeed.
  load();
  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',startAfterAccess,{once:true});
  }else{
    startAfterAccess();
  }

  window.ProgramStudioModularAppShell={
    apps:APPS,
    appKey:key,
    reload:()=>{started=false;load();},
    openQuickAction,
    stage:'modular-pdf-shell-v7-cleanup',
