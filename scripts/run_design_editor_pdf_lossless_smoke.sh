...
PORT="${DESIGN_EDITOR_PDF_LOSSLESS_SMOKE_PORT:-4175}"
...
if ! grep -q 'PASS: two-surface lossless PNG-backed 300DPI PDF export orchestration' "$DOM_OUT"; then echo "Lossless PDF completion marker is missing." >&2; cat "$DOM_OUT" >&2; exit 1; fi
for marker in 'data-lossless-pages="2"' 'data-lossless-images="2"' 'data-lossless-gate="pdf"' 'data-lossless-profile="lossless"' 'data-lossless-width="2551"' 'data-lossless-height="3579"'; do
  if ! grep -q "$marker" "$DOM_OUT"; then echo "Lossless PDF marker missing: $marker" >&2; cat "$DOM_OUT" >&2; exit 1; fi
done

echo "Design editor lossless PDF browser smoke passed using $BROWSER"
