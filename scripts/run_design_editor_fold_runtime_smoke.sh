...
for marker in 'data-fold-runtime-leaflet2="1"' 'data-fold-runtime-leaflet3="2"' 'data-fold-runtime-portrait="2"' 'data-fold-runtime-ensure="ready"'; do
  if ! grep -q "$marker" "$DOM_OUT"; then echo "Fold runtime marker missing: $marker" >&2; cat "$DOM_OUT" >&2; exit 1; fi
done
if ! grep -q 'PASS: 2단 1개, 3단 2개 접지선과 가로·세로 방향 전환이 실제 DOM에서 확인됨' "$DOM_OUT"; then echo "Fold runtime completion marker missing." >&2; cat "$DOM_OUT" >&2; exit 1; fi
