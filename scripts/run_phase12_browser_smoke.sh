...
PORT="${PHASE12_SMOKE_PORT:-4197}"
OUT_DIR="${PHASE12_BROWSER_SMOKE_OUT:-$ROOT_DIR/browser-smoke-artifacts}"
SERVER_LOG="$OUT_DIR/phase12-smoke-server.log"
...
if [[ -z "$BROWSER" ]]; then echo "Headless Chrome/Chromium executable not found for Phase 12 smoke." >&2; rm -rf "$PROFILE_DIR"; exit 1; fi
...
URL="http://127.0.0.1:$PORT/tests/browser/editor-tool-rail-v6-smoke.html"
OUT="$OUT_DIR/editor-tool-rail-v6-smoke-dom.html"
...
if ! grep -q 'data-phase12-smoke="pass"' "$OUT"; then
  echo "Editor tool rail Phase 12 smoke failed." >&2
  cat "$OUT" >&2
...
echo "Editor tool rail Phase 12 smoke passed using $BROWSER"
