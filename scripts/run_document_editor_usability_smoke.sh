...
PORT="${DOCUMENT_EDITOR_USABILITY_SMOKE_PORT:-4189}"
OUT_DIR="${DOCUMENT_EDITOR_BROWSER_SMOKE_OUT:-$ROOT_DIR/browser-smoke-artifacts}"
DOM_OUT="$OUT_DIR/document-editor-usability-smoke-dom.html"
SERVER_LOG="$OUT_DIR/document-editor-usability-smoke-server.log"
...
if [[ -z "$BROWSER" ]]; then echo "Headless Chrome/Chromium executable not found for document usability smoke." >&2; rm -rf "$PROFILE_DIR"; exit 1; fi
...
trap cleanup EXIT
URL="http://127.0.0.1:$PORT/tests/browser/document-editor-usability-smoke.html"
...
for marker in 'data-document-zoom="125"' 'data-document-template="meeting"' 'data-document-paste="true"' 'data-document-paste-prevented="true"' 'data-document-template-count="3"' 'data-document-usability-stage="document-editor-usability-stage3"'; do
  if ! grep -q "$marker" "$DOM_OUT"; then echo "Document editor usability marker missing: $marker" >&2; cat "$DOM_OUT" >&2; exit 1; fi
done

echo "Document editor usability browser smoke passed using $BROWSER"
