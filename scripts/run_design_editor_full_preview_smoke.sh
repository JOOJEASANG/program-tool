...
PORT="${DESIGN_EDITOR_FULL_PREVIEW_SMOKE_PORT:-4199}"
OUT_DIR="${DESIGN_EDITOR_BROWSER_SMOKE_OUT:-$ROOT_DIR/browser-smoke-artifacts}"
DOM_OUT="$OUT_DIR/design-editor-full-preview-smoke-dom.html"
SERVER_LOG="$OUT_DIR/design-editor-full-preview-smoke-server.log"
...
if [[ -z "$BROWSER" ]]; then echo "Headless Chrome/Chromium executable not found for full preview smoke." >&2; rm -rf "$PROFILE_DIR"; exit 1; fi
...
trap cleanup EXIT
URL="http://127.0.0.1:$PORT/tests/browser/design-editor-full-preview-smoke.html"
...
if ! grep -q 'data-full-preview-stage="two-pane-full-preview-workspace-v1"' "$DOM_OUT"; then echo "Full preview stage marker is missing." >&2; cat "$DOM_OUT" >&2; exit 1; fi

echo "Design editor full preview browser smoke passed using $BROWSER"
