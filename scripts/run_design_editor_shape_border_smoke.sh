...
PORT="${DESIGN_EDITOR_SHAPE_BORDER_SMOKE_PORT:-4195}"
OUT_DIR="${DESIGN_EDITOR_BROWSER_SMOKE_OUT:-$ROOT_DIR/browser-smoke-artifacts}"
DOM_OUT="$OUT_DIR/design-editor-shape-border-smoke-dom.html"
SERVER_LOG="$OUT_DIR/design-editor-shape-border-smoke-server.log"
...
if [[ -z "$BROWSER" ]]; then echo "Headless Chrome/Chromium executable not found for shape border smoke." >&2; rm -rf "$PROFILE_DIR"; exit 1; fi
...
trap cleanup EXIT
URL="http://127.0.0.1:$PORT/tests/browser/design-editor-shape-border-smoke.html"
...
if ! grep -q 'data-shape-border-smoke="pass"' "$DOM_OUT"; then
  echo "Design editor shape border browser smoke failed." >&2
  cat "$DOM_OUT" >&2
...
echo "Design editor shape border browser smoke passed using $BROWSER"