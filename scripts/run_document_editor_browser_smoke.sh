...
if [[ -z "$BROWSER" ]]; then echo "Headless Chrome/Chromium executable not found for document editor smoke." >&2; rm -rf "$PROFILE_DIR"; exit 1; fi
...
if ! grep -q 'data-document-editor-smoke="pass"' "$DOM_OUT"; then echo "Document editor browser smoke failed." >&2; cat "$DOM_OUT" >&2; echo "----- HTTP server log -----" >&2; cat "$SERVER_LOG" >&2; exit 1; fi
if ! grep -q 'PASS: edit, format, table, image, autosave restore, print and new document' "$DOM_OUT"; then echo "Document editor completion marker missing." >&2; cat "$DOM_OUT" >&2; exit 1; fi
for marker in 'data-document-bold="true"' 'data-document-table="2x3"' 'data-document-image="true"' 'data-document-restore="true"' 'data-document-print="true"' 'data-document-stage="document-editor-core-stage1"'; do
  if ! grep -q "$marker" "$DOM_OUT"; then echo "Document editor smoke marker missing: $marker" >&2; cat "$DOM_OUT" >&2; exit 1; fi
done
